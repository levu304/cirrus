...
///
/// ```ignore
/// let result = ListBucketResult { name: "my-bucket".into(), .. };
/// let xml = serialize(&result, "ListBucketResult");
/// assert!(xml.starts_with("<ListBucketResult xmlns=\"http://s3.amazonaws.com/doc/2006-03-01/\">"));
/// ```
pub fn serialize<T: serde::Serialize>(value: &T, root_name: &str) -> String {
    let body = to_xml_string(value);
    inject_xmlns(&body, root_name)
}
...
mod tests {
    ...
    fn test_serialize_injects_xmlns_on_root() {
        ...
        let xml = serialize(&value, "SimpleRoot");
    ...
    fn test_serialize_xmlns_on_root_only_not_children() {
        ...
        let xml = serialize(&value, "RootWithChild");
    ...
    fn test_inject_xmlns_no_attributes() {
        ...
        assert!(output.starts_with("<ListBucketResult xmlns=\"http://s3.amazonaws.com/doc/2006-03-01/\">"));
